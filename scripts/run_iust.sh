...
if [ -z $1 ]; then
  echo "[USAGE] ./run_iust.sh [iust|jpos] [f|l] [l| ]"
  echo "        iust: iust_ctrl, jpos: jpos_ctrl, spi: spi_ctrl
  ...
  if [ "$1" = "iust" ]; then
    ...
    echo "[USAGE] ./run_iust.sh [mpc|jpos|spi] [f|l] [l| ]"
    echo "        mpc: iust_ctrl, jpos: jpos_ctrl, spi: spi_ctrl
          f: param from file, l: param form lcm
          l: create terminal log, empty: no log"
    exit 1
  fi
