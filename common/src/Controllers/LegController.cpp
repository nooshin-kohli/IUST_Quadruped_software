...
/*!
 * Update the "leg data" from a CAN Interface message
 */
template <typename T>
void LegController<T>::updateData(const CANData* canData) {
  for (int leg = 0; leg < 4; leg++) {
    // q:
...
/*!
 * Update the "leg command" for the CAN interface message
 */
template <typename T>
